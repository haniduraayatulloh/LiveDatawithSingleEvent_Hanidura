...
import retrofit2.Callback // PERBAIKAN: Import Callback dari Retrofit2
import retrofit2.Response   // PERBAIKAN: Import Response dari Retrofit2
...
class MainViewModel : ViewModel() {
    ...
    private fun findRestaurant() {
        ...
        // PERBAIKAN: Memanggil ApiService dengan cara yang standar
        val client = ApiConfig.getApiService().getRestaurant(RESTAURANT_ID)
        // PERBAIKAN: Menggunakan object Callback dari Retrofit
    ...
    fun postReview(review: String) {
        ...
        client.enqueue(object : Callback<PostReviewResponse> {
            override fun onResponse(call: Call<PostReviewResponse>, response: Response<PostReviewResponse>) {
                _isLoading.value = false
                if (response.isSuccessful) {
                    // Refresh review list with the response from the server
